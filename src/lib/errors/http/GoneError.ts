import type { Dict } from 'typesdk/types';

import { ExtendedSerializableError, SerializedError } from './core';


export class GoneError extends ExtendedSerializableError {
  public override readonly statusCode = 410 as const;
  public override readonly name = 'GoneError' as const;

  constructor(message: string, action?: string, location?: string, context?: Dict<any>, errors?: SerializedError[]) {
    super(message, {
      action,
      errors,
      context,
      location,
      statusCode: 410 as const,
    });
  }
}

export default GoneError;