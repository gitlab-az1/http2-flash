import type { Dict } from 'typesdk/types';

import { ExtendedSerializableError, SerializedError } from './core';


export class NetworkAuthenticationRequiredError extends ExtendedSerializableError {
  public override readonly statusCode = 511 as const;
  public override readonly name = 'NetworkAuthenticationRequiredError' as const;

  constructor(message: string, action?: string, location?: string, context?: Dict<any>, errors?: SerializedError[]) {
    super(message, {
      action,
      errors,
      context,
      location,
      statusCode: 511 as const,
    });
  }
}

export default NetworkAuthenticationRequiredError;