...
export class IPv4 {
  ...
  public get octets(): number[] {
    return this.#value.split('.').map(parseInt);
  }
...
export class IPv6 {
   ...
   */
  public get octets(): number[] {
    return this.#value.split(':').map(block => parseInt(block, 16));
  }
  ...
  public static from(value?: any): Either<InvalidIPAddress, IPv6> {
    ...
    if(Array.isArray(value)) {
      if(value.length !== 8) return result = left(new InvalidIPAddress('Invalid block count', 422, {
        expected: 8,
        received: value.length,
      }));

      if(!value.every(isString)) return result = left(new InvalidIPAddress('Invalid block type', 422, {
        expected: 'string',
        received: 'any',
      }));

      if(!value.every(block => /^[0-9a-fA-F]{1,4}$/.test(block))) return result = left(new InvalidIPAddress('Invalid block format', 422, {
        expected: '0-9a-fA-F',
        received: value.toString(),
      }));
      ...
      result = this.validate(value) ?
        right(new IPv6(value)) :
        left(new InvalidIPAddress('Invalid or malformed IPv6 address'));
    }
...
export function extractIPFromRequest(request: MaybeRequestWithHeaders): IPv4 | IPv6 {
  ...
  if(isPlainObject(request.headers)) {
    ...
    throw new TypeError('Failed to resolve IP address from request: object malformed');
  }
